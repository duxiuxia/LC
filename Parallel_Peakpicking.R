...
if (window_linux)
{
	...
	Code_Path          <- "/home/xdu4/testLECO/LCMS"             # "C:\\lc_R_test\\test" 
	Input_Data_Path    <- "/home/xdu4/testLECO/Input"      # "C:\\lc_R_test\\lc_ms_file"
	Output_Data_Path   <- "/home/xdu4/testLECO/Output"           # "C:\\lc_R_test\\lc_ms_output"
	Configure_FilePath <- "/home/xdu4/testLECO/Configure"  # "C:\\lc_R_test\\lc_ms_configure"
}
...
# ======================================================
#FileName_List <-list.files(path=Output_Data_Path, pattern = file_extend)  #,full.names=TRUE   ,
FileName_List <-list.files(path=Output_Data_Path, pattern = "RData")
...
if((window_linux==1)||(PARAMS$Cluster_num<2))  #Windows system or linux system but Cluster node Number <2, use serial process to run the LC-MS data 
{
	...
	in_file_name <- FileName_List[[2]]	
	in_file_full_name <- paste(Output_Data_Path, in_file_name, sep="/")
	...
	ind <- regexpr("_EIC", in_file_name)
	out_file_name <- substring(in_file_name, first=1, last=(ind-1))
	out_file_name <- paste(out_file_name, "_peak_list.csv", sep="")
	...
	EIC_Groups<-clusterSplit(cl,EIC_Extracted_mzdata)
#	clusterExport(cl,"EIC_FillMissing_Smoothing")
#	clusterExport(cl,"Chromatogram_WindowFilter_Smooth")
#	clusterExport(cl,"BaseLine_Remove")
#	clusterExport(cl,"PeakDetect_CentWave")
#	clusterExport(cl,"wavCWT_Modify")
#	clusterExport(cl,"wavCWTTree_Modify")
#	clusterExport(cl,"wavCWTPeaks_Modify")
#	clusterExport(cl,"Boundary_Extending")
#	clusterExport(cl,"Peak_Area_Calculation")
	clusterExport(cl,"PeakDetect_Process_parallel")
...
#		clusterExport(cl,"Par_Process_mzdata_File")
		peak_lists <- clusterApply(cl,EIC_Groups, PeakDetect_Process_parallel, scan_number_mzdata, Code_Path, sep_char)#parallel version
		
#		peak_lists<-unlist(peak_lists,recursive=F)
		peak_lists<-do.call(rbind,peak_lists)
		write.csv(peak_lists,file=paste(Output_Data_Path, out_file_name, sep="/"),row.names=F) 
